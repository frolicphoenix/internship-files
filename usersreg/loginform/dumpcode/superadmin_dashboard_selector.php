    ...
    <?php foreach ($admin_types as $admin_type): ?>
        <section class="admin-type">
            ...
            <div class="privilege-group">
                <h4>Genders:</h4>
                <select name="privileges[<?php echo $admin_type['admin_type_id']; ?>][gender][]" multiple>
                    <?php foreach ($genders as $gender): ?>
                        <option value="<?php echo $gender['id']; ?>"
                            <?php echo (isset($current_privileges[$admin_type['admin_type_id']]['gender']) && 
                                        in_array($gender['id'], $current_privileges[$admin_type['admin_type_id']]['gender'])) ? 'selected' : ''; ?>>
            ...
            <div class="privilege-group">
                <h4>Top Sizes:</h4>
                <select name="privileges[<?php echo $admin_type['admin_type_id']; ?>][top_size][]" multiple>
                    <?php foreach ($top_sizes as $size): ?>
                        <option value="<?php echo $size['id']; ?>"
                            <?php echo (isset($current_privileges[$admin_type['admin_type_id']]['top_size']) && 
                                        in_array($size['id'], $current_privileges[$admin_type['admin_type_id']]['top_size'])) ? 'selected' : ''; ?>>
